...
import { loadSeenActivityIds, notificationStateExists, saveSeenActivityIds } from '../src/history.js';
...
test('notification state is stored and loaded atomically', async () => {
  ...
  try {
    ...
    assert.equal(await notificationStateExists(directory), true);
...
test('legacy notified reports are migrated when state is missing', async () => {
  ...
  try {
    ...
    assert.equal(await notificationStateExists(directory), false);
    const loaded = await loadSeenActivityIds(directory);
