...
async function main() {
  ...
  const detailResults = await mapLimit(ids, CONCURRENCY, async (id) => {
    try {
  ...
  for (const result of detailResults) {
    if (!result.error) detailById.set(result.id, result.data);
  }
  const activities = Array.from(byId.values()).map(activity => {
    const detail = detailById.get(activity.offlineActivityId) || {};
  ...
  console.log(`[update-data] wrote ${outPath} activities=${activities.length}`);
}
